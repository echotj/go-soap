...
type ReportView struct {
	XMLName xml.Name `xml:"http://wssmcommupper/ reportView"`
	Arg0 string `xml:"arg0,omitempty"`

}

type ReportViewResponse struct {
	XMLName xml.Name `xml:"http://wssmcommupper/ reportViewResponse"`

	Return_ int32 `xml:"return,omitempty"`
}

type WSSmCommUpper struct {
	client *SOAPClient
}

func NewWSSmCommUpper(url string, tls bool, auth *BasicAuth) *WSSmCommUpper {
	if url == "" {
		url = ""
	}
	client := NewSOAPClient(url, tls, auth)

	return &WSSmCommUpper{
		client: client,
	}
}

func NewWSSmCommUpperWithTLSConfig(url string, tlsCfg *tls.Config, auth *BasicAuth) *WSSmCommUpper {
	if url == "" {
		url = ""
	}
...
func (service *WSSmCommUpper) AddHeader(header interface{}) {
	service.client.AddHeader(header)
}

// Backwards-compatible function: use AddHeader instead
func (service *WSSmCommUpper) SetHeader(header interface{}) {
	service.client.AddHeader(header)
}

func (service *WSSmCommUpper) ReportView(request *ReportView) (*ReportViewResponse, error) {
	response := new(ReportViewResponse)
